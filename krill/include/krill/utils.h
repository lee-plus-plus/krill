...
template <typename T>
inline std::set<T> to_set(std::vector<T> v) {
...
template <typename T>
inline std::vector<T> get_top(std::stack<T> s, size_t size) {
    assert(size >= 0);
    assert(s.size() >= size);
    std::vector<T> v(size);
...
template <class>
inline constexpr bool is_set = false;
template <class T, class A>
inline constexpr bool is_set<std::set<T, A>> = true;
